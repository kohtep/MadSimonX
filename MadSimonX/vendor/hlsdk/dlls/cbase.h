...
public:
	...
	void SetEternalThink()
	{
		if (pev->movetype == 7)
		{
			pev->nextthink = pev->ltime + 1000000.0f;
			m_fPevNextThink = pev->nextthink;
		}

		for (CBaseEntity *pChild = m_pChildMoveWith; pChild; pChild->m_pSiblingMoveWith)
			SetEternalThink();
	}
	...
	//LRC - loci
	virtual Vector *CalcPosition(CBaseEntity *pLocus, Vector *OUTresult) {}
	virtual Vector *CalcVelocity(CBaseEntity *pLocus, Vector *OUTresult) {}
	...
	virtual STATE GetState(CBaseEntity *pEnt) {}
